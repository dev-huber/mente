...
class CacheService {
    ...
    async initializeRedis() {
        try {
            ...
            if (!redisUrl) {
                logger_1.logger.warn('Redis não configurado, usando cache local apenas', {
                ...
                socket: {
                    reconnectStrategy: (retries) => {
                        if (retries > this.maxReconnectAttempts) {
                            logger_1.logger.error('Max reconnect attempts reached', new Error('Redis connection failed'), {
    ...
    async get(key) {
        ...
        try {
            ...
            if (this.isConnected && this.client) {
                try {
                    ...
                    if (value) {
                        ...
                        logger_1.logger.debug('Cache hit (Redis)', {
                            operation: 'get',
                            service: 'CacheService',
                            duration,
                            metadata: { key },
                    ...
                    logger_1.logger.warn('Redis get failed, falling back to local cache', {
                        operation: 'get',
                        service: 'CacheService',
                        metadata: { key, error: redisError.message },
                    });
                }
            }
            ...
            if (localEntry && localEntry.expiry > Date.now()) {
                const duration = timer();
                logger_1.logger.debug('Cache hit (local)', {
                    operation: 'get',
                    service: 'CacheService',
                    duration,
                    metadata: { key },
        ...
        try {
            ...
            if (this.isConnected && this.client) {
                try {
                    ...
                    const duration = timer();
                    logger_1.logger.debug('Cache set (Redis + local)', {
                        operation: 'set',
                        service: 'CacheService',
                        duration,
                    ...
                    logger_1.logger.warn('Redis set failed, saved to local cache only', {
                        operation: 'set',
                        service: 'CacheService',
                        metadata: { key, error: redisError.message },
                    });
                ...
                const duration = timer();
                logger_1.logger.debug('Cache set (local only)', {
                    operation: 'set',
                    service: 'CacheService',
                    duration,
                    metadata: { key, ttl },
    ...
    async invalidate(key) {
        try {
            ...
            logger_1.logger.debug('Cache invalidated', {
    ...
    async invalidateByTag(tag) {
        try {
            ...
            logger_1.logger.error('Cache invalidate by tag error', error, {
                operation: 'invalidateByTag',
                service: 'CacheService',
                metadata: { tag },
            });
    ...
    async addToTags(key, tags) {
        ...
        try {
            ...
            logger_1.logger.warn('Failed to add cache tags', {
                operation: 'addToTags',
                service: 'CacheService',
                metadata: { key, tags },
            });
    ...
    cleanupLocalCache() {
        ...
        if (this.localCache.size > maxLocalCacheSize) {
            ...
            logger_1.logger.warn('Local cache size limit reached, removed oldest entries', {
