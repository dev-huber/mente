...
export interface LieDetectionResult {
  lieScore: number; // 0-100 (100 = certeza de mentira)
...
export class LieDetectionService {
  ...
  async detectLie(input: LieDetectionInput): Promise<LieDetectionResult> {
    ...
    try {
      ...
      logger.info('Análise de mentira concluída', {
        operation: 'detectLie',
      ...
      logger.metric('lie_detection_score', lieScore);
      logger.metric('lie_detection_confidence', confidence);
      ...
      logger.error('Falha na detecção de mentira', error, {
        operation: 'detectLie',
        service: 'LieDetectionService',
      });
      ...
      throw new BusinessLogicError(
        ...
        ErrorCode.LIE_DETECTION_FAILED,
        { operation: 'detectLie' }
      );
    }
  }
  
  private validateInput(input: LieDetectionInput): void {
  ...
  private async analyzeLinguisticPatterns(transcription: any): Promise<number> {
    ...
    if (tenseChanges > 3) score += 15;
    else if (tenseChanges > 1) score += 7;
    
    // Analisar distanciamento (uso de terceira pessoa)
    const distancing = this.detectDistancing(transcription.text);
    if (distancing) score += 15;
    
    return Math.min(100, score);
  }
  
  private async analyzeVocalPatterns(vocalAnalysis: any): Promise<number> {
    let score = 0;
    
    // Alta variação de pitch indica stress
    if (vocalAnalysis.pitchStd > 50) score += 25;
    else if (vocalAnalysis.pitchStd > 30) score += 15;
    ...
    // Muitas pausas
    if (vocalAnalysis.pauseRatio > 0.4) score += 25;
    else if (vocalAnalysis.pauseRatio > 0.25) score += 15;
    
    // Variação de energia (nervosismo)
    if (vocalAnalysis.energyStd > 40) score += 20;
    else if (vocalAnalysis.energyStd > 25) score += 10;
    
    // Pitch anormalmente alto (tensão)
    if (vocalAnalysis.pitchMean > 250) score += 10;
    
    return Math.min(100, score);
  }
  
  private async analyzeSentiment(sentiment: any): Promise<number> {
    let score = 0;
    
    // Negatividade excessiva
    if (sentiment.negative > 0.6) score += 30;
    else if (sentiment.negative > 0.4) score += 15;
    ...
    if (emotionalVariance < 0.1) score += 20; // Emoções muito neutras podem ser falsas
    
    return Math.min(100, score);
  }
  
  private async analyzeConsistency(input: LieDetectionInput): Promise<number> {
    let score = 0;
    ...
    if (inconsistency > 40) score += 30;
    else if (inconsistency > 20) score += 15;
    
    // Verificar consistência temporal
    const temporalInconsistencies = this.checkTemporalConsistency(input.transcription.text);
    if (temporalInconsistencies > 2) score += 25;
    else if (temporalInconsistencies > 0) score += 12;
    
    // Verificar contradições
    const contradictions = this.findContradictions(input.transcription.text);
    if (contradictions > 0) score += 30;
    
    return Math.min(100, score);
  }
  
  private calculateFinalScore(components: any): number {
    const weightedScore = 
      components.linguistic * this.weights.linguistic +
  ...
  private classify(lieScore: number, confidence: number): 'truth' | 'lie' | 'uncertain' {
  ...
  private generateExplanation(data: any): { explanation: string; keyIndicators: string[] } {
  ...
  private generateSuggestions(confidence: number, input: any): string[] {
  ...
  private detectTenseChanges(text: string): number {
  ...
  private detectDistancing(text: string): boolean {
  ...
  private checkTemporalConsistency(text: string): number {
  ...
  private findContradictions(text: string): number {
